# 13 Product Principles And Page Checklists

## Goal
- Turn the product thesis into ship criteria that can be applied to every page family.
- Keep operator trust high when inspection pressure is high.
- Make authority logic, proof requirements, and next action clearer than vendor routing.

## Product identity
- KitchenComplianceHub is a local compliance and next-action product for commercial kitchen operators.
- It is not a generic restaurant operations blog.
- It is not a national compliance explainer that smooths over local differences.
- It is not a vendor directory with compliance copy wrapped around it.

## Core invariants
- Canonical indexed pages must resolve to a city, utility, fire authority, or other local rule holder.
- The first visit can be issue-first, but the resolved destination must become city-first and authority-aware.
- Every local verdict page must answer the same operator questions in the same order:
  - what the authority requires
  - what proof must stay on site
  - what fails inspection or triggers enforcement risk
  - what action the operator should take next
- Official guidance, sponsor layers, and public provider inventory must stay visually and structurally separate.
- Finder trust depends on visible evidence quality, not on card count alone.
- If local source quality or provider coverage is weak, the route should fall back to noindex or to a verification workflow instead of pretending certainty.
- Repeat utility matters more than content breadth. Pages should help an operator come back with a binder, manifest, due date, or inspection problem.
- Freshness is a trust feature. `verifiedOn`, `nextReviewOn`, and evidence recency are part of the product, not metadata trivia.

## Persona lens

### Restaurant owner
- Wants the shortest path to compliance risk reduction.
- Needs a clear answer on what must be fixed now versus what can wait.
- Judges trust by whether the page sounds official and operational, not promotional.

### Kitchen manager
- Needs a page that can be used on the floor before an inspection or service visit.
- Cares about proof-on-site, tags, manifests, stickers, logs, and contact steps.
- Rejects pages that are vague about failure conditions or required records.

### Multi-location operator
- Needs the product to preserve city differences instead of collapsing them into generic advice.
- Values repeatable page structure so different local markets can be checked fast.
- Cares about route freshness and source dates because stale local guidance creates real risk.

### Operator under inspection pressure
- Wants to start from the issue on the desk, not from a browse-heavy content tree.
- Needs the next action to be obvious within seconds.
- Will distrust the page if vendor blocks appear before the rule summary.

## Global page test
- Can the operator identify the local authority in one screen?
- Can the operator see the required proof-on-site without reading the full page?
- Can the operator tell what fails inspection or blocks approval?
- Can the operator see the next operational step without being forced into a lead form?
- Can the operator distinguish official guidance from sponsor or vendor content immediately?
- If the page contains providers, can the operator see why each provider appears there?

## Entry logic

### Home and top-level entry
- Start with the issue on the desk:
  - grease, hauling, and manifests
  - hood cleaning, suppression, and tags
  - inspection prep and on-site records
- Resolve into city-aware or authority-aware destinations as early as possible.
- Do not make vendor discovery the primary first-click behavior.
- Explain the product in operator terms:
  - choose city
  - check rule
  - keep proof on site
  - take the next action

### Canonical local destinations
- Once the operator picks a city or lands on a local page, the page must stay anchored to that local authority context.
- National or evergreen guidance may support understanding, but it must never outrank the local rule summary.

## Page-family checklists

### Home page

#### Must do
- Explain that the product covers FOG, hood, and inspection-prep workflows.
- Let the operator begin from the operational issue, not from a generic content taxonomy.
- Push the user toward city selection or local authority resolution quickly.
- Show that the site is authority-backed and operator-oriented.
- Make the separation between rule clarity and vendor routing visible.

#### Should do
- Offer direct paths for first-visit operators and repeat operators.
- Show the repeated utility loop: rule, proof, binder, service action, re-check.
- Keep copy compact enough for an anxious operator to scan quickly.

#### Must not do
- Read like a generic restaurant blog homepage.
- Lead with sponsor cards, provider logos, or marketplace language.
- Promise broad national answers that ignore city differences.

#### No-ship signals
- The hero can be mistaken for a marketing site for service vendors.
- The first clear CTA is a vendor directory click.
- The page does not explain what proof or inspection-prep value the product provides.

### Canonical FOG rule pages

#### Must do
- Name the local rule holder.
- State what the operator must maintain, pump, clean, report, or document.
- State whether the locality uses an approved hauler list, a permitted hauler workflow, or an operator verification workflow.
- Show the exact proof to keep on site:
  - manifests
  - service records
  - interceptor logs
  - permit or program records when applicable
- Explain common failure or enforcement triggers.
- End with the next action that matches the local state.

#### Must not do
- Generalize another city's FOG rules onto this page.
- Claim an approved list where no official list exists.
- Bury the local source stack below vendor content.

### Approved hauler and verification workflow pages

#### Must do
- Distinguish clearly between:
  - officially published approved lists
  - publicly discoverable providers without official approval language
  - self-verification workflows where the operator must confirm credentials
- Explain the operator's verification job:
  - what list or authority page to check
  - what manifest or permit detail to confirm
  - what record to keep after service
- Show evidence quality on every provider record when providers are displayed.
- Explain what the product knows, what it does not know, and what the operator still needs to verify.

#### Must not do
- Flatten official approval and public directory inclusion into the same visual status.
- Imply city endorsement because a provider is listed on KitchenComplianceHub.
- Hide the verification workflow behind lead capture.

#### No-ship signals
- Provider cards read like ads before the authority logic is clear.
- Evidence quality is not visible.
- The route has thin provider coverage and no useful verification workflow.

### Canonical hood requirement pages

#### Must do
- Anchor the page to the responsible local fire authority, building authority, or adopted code path.
- Separate hood cleaning cadence, suppression system inspection, and tag or certificate requirements when they differ.
- Tell the operator exactly what proof must stay on site.
- Explain the conditions that commonly fail inspection:
  - overdue cleaning
  - missing tag or certificate
  - inaccessible or incomplete records
  - mismatched system condition versus posted proof
- Give the next action in operational language.

#### Must not do
- Collapse suppression inspection and hood cleaning into one invented rule.
- Use a generic national cadence without local support.
- Put hood cleaner routing above the local requirement summary.

### Inspection checklist and inspection-prep pages

#### Must do
- Use stable, operator-friendly taxonomy instead of loose mixed lists.
- Make on-site proof the center of the page.
- Name the local inspection authority and inspection type.
- Show common fail reasons in concrete language.
- Help the operator prepare the binder or records set before listing services.

#### Recommended taxonomy
- extinguishers and suppression
- records binder
- exit and egress
- electrical safety
- local add-ons required by the authority

#### Must not do
- Read like a long generic safety article.
- Mix optional best practices with likely fail items without labeling the difference.
- Route into vendors before the checklist is usable on its own.

### Finder pages

#### Role in the product
- Finder pages are secondary surfaces that support the next action after rule clarity.
- They can help sponsor sales, but only if trust survives.

#### Must do
- Explain why each provider appears on the page.
- Show evidence quality, source type, and cited authority link when available.
- Show service geography and service scope clearly enough to prevent bad leads.
- Keep sponsor treatment clearly separate from public or authority-backed evidence blocks.
- Stay indexable only when source quality and provider depth are strong enough to help an operator.

#### Must not do
- Behave like a thin lead-gen directory.
- Give all providers the same trust treatment when the evidence tiers differ.
- Use local-rule copy as a pretext for a weak directory.

#### Indexing gate
- `index` only when:
  - local rule context is strong
  - provider coverage is meaningfully useful
  - evidence quality is visible
  - the route helps the operator act without confusion
- `noindex` when:
  - coverage is thin
  - evidence is mostly contact-only
  - sponsor pressure would outrun trust

### Evergreen guides

#### Role in the product
- Evergreen guides support understanding and capture broader intent, but they should feed operators back into local rule pages.

#### Must do
- State clearly that local requirements vary by authority.
- Link back to city-aware rule pages, verification workflows, or operator tools.
- Stay useful for repeat operators, not just top-of-funnel search traffic.

#### Must not do
- Compete with canonical local pages for local intent.
- Overpromise generic best practice as if it were binding local guidance.
- Expand into broad restaurant operations content that weakens the wedge.

### Noindex operator tools

#### Role in the product
- Operator tools are retention surfaces for repeat use under real operational pressure.
- They should reduce binder chaos, record loss, and due-date misses.

#### Must do
- Be usable for real tasks:
  - grease logs
  - hood record binders
  - inspection reminder planning
- Preserve local context where possible.
- Reinforce proof-on-site behavior, not generic content consumption.
- Stay visibly separate from indexed editorial surfaces when they are utility-first tools.

#### Must not do
- Become thin SEO pages with download buttons.
- Replace authority guidance with internal templates alone.
- Hide the real operator task behind unnecessary friction.

## Sponsor and vendor separation rules
- Official requirement blocks must render before sponsor blocks on local rule pages.
- Sponsor blocks must never mimic authority status language.
- Provider evidence labels must reflect the real source tier.
- When no official approval exists, the UI should say verification is still required.
- Commercial pressure is never a reason to invent certainty.

## Ship and no-ship framework

### Ship
- The page is locally grounded.
- The authority is visible.
- Proof-on-site is explicit.
- Fail conditions are concrete.
- Next action is operationally clear.
- Sponsor or vendor layers are visibly subordinate.

### Hold for revision
- The page is locally relevant but still reads like generic guidance.
- The next action is visible but not specific enough.
- Provider evidence exists but is not surfaced clearly enough.
- The route is useful but not yet trustworthy enough to index.

### Do not ship
- The page implies official approval without an official source.
- The page hides local uncertainty behind confident generic copy.
- The page pushes directory or sponsor conversion before rule clarity.
- The operator cannot tell what document to keep on site.

## Common failure modes
- Vendor-first information architecture that makes the product feel like a directory.
- Nationalized advice that erases city or authority differences.
- Source stacks that sit too low on the page to support trust when the core claim is read.
- Provider cards that do not explain evidence quality.
- Inspection pages that list tasks without clarifying likely fail items.
- Tools that are useful in theory but disconnected from local rule or proof-on-site behavior.
- Indexed finder pages kept alive after local source quality or provider density drops below trust threshold.

## Working rule for future decisions
- Favor operational clarity over content volume.
- Favor local authority logic over generic advice.
- Favor repeat utility over one-off SEO traffic.
- Favor trust preservation over short-term sponsor surface expansion.
